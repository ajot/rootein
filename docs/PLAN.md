...
- `rails new rootein --css=tailwind --database=postgresql`
- Generate the `Rootein` model (just `name:string` for now)
- Show one rootein on a page — ugly, no auth, no styling

### Phase 2: The Calendar
- Generate `Completion` model (rootein_id, completed_on)
- Build a simple calendar view for a single rootein (ERB, no JS yet)
